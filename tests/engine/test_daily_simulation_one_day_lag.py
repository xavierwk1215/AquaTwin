"""Integration test for one-day-lag mineralization."""
...
def test_current_day_feed_does_not_mineralize_on_same_day() -> None:
    """Mineralize only opening Organic-N, excluding today's feed."""
    ...
    context = StepContext(
        ...
        frozen_parameter_set=FrozenParameterSet(
            ...
            organic_n_mineralization_fraction_per_day=0.50,
    ...
    assert metrics["total_feed_nitrogen_mg"] == pytest.approx(
        100.0
    )
    ...
    assert result.new_state.tan_mass_mg == pytest.approx(50.0)

    same_day_mineralization_mg = (
