"""Tests for StepContextTANOxidationEngine."""
...
def make_context(
    *,
    tan_mass_mg: float = 100.0,
    nitrite_mass_mg: float = 20.0,
    fraction: float = 0.5,
) -> StepContext:
    return StepContext(
        previous_state=SimulationState(
            organic_n_mass_mg=50.0,
            tan_mass_mg=tan_mass_mg,
        ...
        frozen_parameter_set=FrozenParameterSet(
            ...
            tan_oxidation_fraction_per_day=fraction,
        ),
    )


def test_partial_tan_oxidation() -> None:
    ...
    assert context.working_state.tan_mass_mg == pytest.approx(50.0)
    assert context.working_state.nitrite_mass_mg == pytest.approx(
        70.0
    )
    assert context.metrics[-1].name == "tan_n_oxidized_mg"
...
def test_zero_fraction_changes_nothing() -> None:
    ...
    assert context.working_state.tan_mass_mg == pytest.approx(100.0)
    assert context.working_state.nitrite_mass_mg == pytest.approx(
        20.0
    )
...
def test_full_fraction_oxidizes_all_tan() -> None:
    context = make_context(
        fraction=1.0,
    )
    ...
    assert context.working_state.tan_mass_mg == pytest.approx(0.0)
    assert context.working_state.nitrite_mass_mg == pytest.approx(
        120.0
    )


def test_tan_oxidation_does_not_change_other_nitrogen_pools() -> None:
    context = make_context(
        fraction=0.5,
    )
